...
export function GooeyNav({ items, className, currentPath = '' }: GooeyNavProps) {
  const [hoveredPath, setHoveredPath] = useState<string | null>(null);
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null);

  return (
    <nav className={["relative flex items-center gap-1", className].filter(Boolean).join(" ")}>
      {items.map((item) => {
          ...
          >
            <Link
              ...
              className={[
                ...
                isActive ? "text-[#f59e0b]" : "text-gray-300 group-hover:text-[#f59e0b]"
              ].join(" ")}
            >
              ...
              <span className="relative z-10 flex items-center gap-1">
                {item.label}
                {item.hasDropdown && (
                  <ChevronDown className={["w-4 h-4 transition-transform duration-200", isDropdownActive ? "rotate-180" : ""].join(" ")} />
                )}
              </span>
            </Link>
            ...
            <AnimatePresence>
              {item.hasDropdown && isDropdownActive && (
                <motion.div
                  className="absolute top-full left-0 mt-2 w-64 bg-[#121212] rounded-lg shadow-2xl border border-[#262626] py-2 overflow-hidden z-20"
